...
/**
 * DynaTech-drake Java 21 Project Panama (FFM API) Native Rust Bridge
 */
    ...
    public static void initialize(Path nativeLibPath) {
        try {
            ...
            if (symbol != null) {
                ...
                isNativeLoaded = true;
                LOGGER.info("⚡ [DynaTech] Successfully bound to Slimefun-Rust Native FFM Engine!");
            }
        } catch (Throwable t) {
            LOGGER.warning("⚠️ [DynaTech] Slimefun-Rust native library not loaded: " + t.getMessage());
        }
    }
